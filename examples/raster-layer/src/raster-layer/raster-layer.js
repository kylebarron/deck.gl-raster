...
export default class RasterLayer extends BitmapLayer {
  initializeState() {
    ...
    const fsStr = [
      ...
      // "fs:DECKGL_MUTATE_COLOR(inout uvec4 image, in vec2 coord)",
      // "fs:DECKGL_CREATE_COLOR(inout uvec4 image, in vec2 coord)",
    ]
    for (const str of fsStr) {
      if (!programManager._hookFunctions.includes(str)) {
        programManager.addShaderHook(str);
      }
    }
